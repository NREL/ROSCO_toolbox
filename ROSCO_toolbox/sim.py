...
class Sim():
    ...
    def sim_ws_wd_series(self,t_array,ws_array,wd_array,
        ...
        for i, t in enumerate(t_array):
            ...
            # Call the controller
            gen_torque[i], bld_pitch[i], nac_yawrate[i] = self.controller_int.call_controller(
                    t, dt, bld_pitch[i-1], gen_torque[i-1], gen_speed[i],
                    self.turbine.GenEff/100, rot_speed[i], ws,
                    yaw=nac_yaw[i-1],
                    yawerr=nac_yawerr[i])
        ...
        if make_plots:
            ...
            ax = axarr[2]
            ax.plot(self.t_array, self.nac_yawerr*rad2deg)
